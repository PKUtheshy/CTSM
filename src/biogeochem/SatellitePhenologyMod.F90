...
#include "shr_assert.h"
  ...
  real(r8), private, allocatable :: mci2t(:,:)  ! ci for interpolation (2 months) ! hzc2024
...
contains
  ...
  subroutine interpMonthlyVeg (bounds, canopystate_inst)
    ...
    if (InterpMonths1 /= months(1)) then
       ...
       write(iulog,*) 'interpMonthlyVeg: start to read monthly vegetation data from',fsurdat!test whether load CI data: hzc2024
  ...
  subroutine readAnnualVegetation (bounds, canopystate_inst)
   ...
   !  call check_dim(ncid, 'lsmpft', numpft+1)
    call check_dim(ncid, 'lsmpft', numpft+1) ! hzc2024
  ...
  subroutine readMonthlyVegetation (bounds, &
    ...
    allocate(&
         ...
         mci(bounds%begg:bounds%endg,0:numpft), &     ! hzc2024
